...
describe("xAI subscription Responses", () => {
	...
	it("preserves model capabilities and sends only verified reasoning efforts", async () => {
		...
		// Every xAI route without documented effort levels carries a fixed reasoning contract, so grok-4.3
		// keeps reasoning on and never serializes an effort, requested or not.
		for (const [id, requested, effort] of [
			["grok-4.3", undefined, undefined],
			["grok-4.3", "low", undefined],
			["grok-4.5", "xhigh", "high"],
			...
			// Compat is api-specific: the catalog row carries openai-completions flags the Responses route rebases.
			expect(adapted).toMatchObject({
				...source,
				api: "openai-responses",
				compat: { supportsLongCacheRetention: false },
			});
			expect(adapted.compat).toEqual({ supportsLongCacheRetention: false });
		...
		expect(getSupportedThinkingLevels(getXaiSubscriptionModel(getModel("xai", "grok-build-0.1"))!)).toEqual(["high"]);
	});
