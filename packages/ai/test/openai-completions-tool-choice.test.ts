...
describe("openai-completions tool_choice", () => {
	...
	it("sends no thinking parameter to Prime Inference GLM routes but keeps the z.ai toggle", async () => {
		...
		expect(prime?.thinking).toBeUndefined();
		...
		// Direct z.ai routes carry the fork's thinking object toggle; enable_thinking stays
		// reserved for thinkingFormat "qwen".
		const zai = payloads.get("glm-5.3");
		expect(zai?.thinking).toEqual({ type: "enabled" });
		expect(zai?.enable_thinking).toBeUndefined();
		expect(zai?.reasoning_effort).toBeUndefined();
	});
