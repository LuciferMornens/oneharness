...
describe("Fast mode service tier defaults", () => {
	it.each(["gpt-5.4", "gpt-5.5", "gpt-5.6-sol", "gpt-5.6-luna", "gpt-6-astra"])(
		"defaults ChatGPT-auth %s to priority",
		(id) => {
			const subscription = model("openai-codex", id, "openai-codex-responses");
			expect(supportsFastMode(subscription)).toBe(true);
			expect(defaultServiceTierForModel(subscription)).toBe("priority");
		},
	);
