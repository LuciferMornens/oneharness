...
export const CURSOR_BASE_URL = "https://api.cursor.com/v1";

export const CURSOR_MODEL_ENTRIES: ReadonlyArray<{ id: string; name: string }> = [
	{ id: "auto", name: "Auto" },
...
function cursorContextWindow(id: string, name: string): number {
	const blob = `${id} ${name}`.toLowerCase();
	if (blob.includes("1m") || name.includes("1M")) return 1_000_000;
	if (blob.includes("composer")) return 200_000;
	if (blob.includes("codex") || blob.includes("gpt-5")) return 272_000;
	if (blob.includes("gemini")) return 1_048_576;
	return 200_000;
}

export function getCursorModels(): Model<"openai-completions">[] {
	return CURSOR_MODEL_ENTRIES.map((entry) => {
		return {
			id: entry.id,
			name: entry.name,
			...
			contextWindow: cursorContextWindow(entry.id, entry.name),
			maxTokens: 128000,
			featured: entry.id === "composer-2.5" || entry.id === "auto",
