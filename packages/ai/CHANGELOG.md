...
- Added a built-in `cursor` provider that uses the OpenAI-compatible Cursor API at `https://api.cursor.com/v1` with `CURSOR_API_KEY`, optional `CURSOR_BASE_URL`, and the current Cursor CLI model catalog.
