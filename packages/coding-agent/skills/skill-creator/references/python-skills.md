...
A Python-backed skill is a regular markdown skill that also ships a Python package. Prime Agent installs the package editable into the kernel venv (`~/.prime/agent/kernel-venvs/<schema>-<runtime-hash>` by default, `PRIME_AGENT_KERNEL_VENV` to override with one exact path) and exposes it in the persistent Python kernel, so the agent can call it directly instead of shelling out.
