...
async def spawn(
    ...
    *,
    ...
    ``thinking`` sets the child's reasoning level when the selected model supports it.
    ``effort`` is an accepted alias for ``thinking``.
    Omitting both inherits the parent level clamped to the child model.
...
class _RLMNamespace:
    ...
    async def spawn(
        ...
        *,
        ...
        return await spawn(prompt, name=name, model=model, effort=effort, thinking=thinking)
