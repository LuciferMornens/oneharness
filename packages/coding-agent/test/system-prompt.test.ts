...
describe("buildRlmPrompt", () => {
	...
	test("discovers requested models through a bounded authenticated host search", () => {
		...
		expect(prompt).toContain("`thinking` is an alias for `effort`");
		expect(prompt).toContain("decide whether to retry or omit `model`/`effort`/`thinking`");
	...
	test("guides root agents to orchestrate in parallel, avoid blocking waits, and write clearly", () => {
		...
		expect(root).toContain("assign independent substantive tasks to separate workers");
		expect(root).toContain("let them run in parallel");
		expect(root).toContain("Do not keep the turn open by polling with `time.sleep()` or shell `sleep`");
		expect(root).toContain("do not replace polling with a long blocking `await`");
		expect(root).toContain("otherwise end the turn");
		expect(root).toContain("proactively give regular concise progress updates");
		expect(root).toContain("Lead with user-visible outcomes");
		expect(root).toContain("Use simplified technical English by default for user-facing prose");
		expect(child).not.toContain("As the user-facing root agent");
		expect(child).toContain("Use simplified technical English by default for user-facing prose");
	});
