...
describe("detectInstallMethod", () => {
	...
	test.skipIf(process.platform === "win32")("does not self-update when the npm install path is not writable", () => {
	...
	test.each<[string, string[] | undefined]>([
		...
		expectSelfUpdateCommand(getSelfUpdateCommand("@earendil-works/pi-coding-agent", npmCommand), {
			command: "npm",
	...
	// Windows npm reports `<prefix>\node_modules` as the global root, so the fixture never matches a configured prefix.
	test.skipIf(process.platform === "win32")("npm self-update respects a configured npmCommand", () => {
		const { prefix } = createNpmPrefixInstall();

		expect(getSelfUpdateCommand("@earendil-works/pi-coding-agent", ["npm", "--prefix", prefix])).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@earendil-works/pi-coding-agent"],
			display: `npm --prefix ${prefix} install -g @earendil-works/pi-coding-agent`,
		});
	...
	test("quotes npm self-update display paths", () => {
		...
		const display = `npm --prefix "${prefix}" install -g @earendil-works/pi-coding-agent`;

		const actual = getSelfUpdateCommand("@earendil-works/pi-coding-agent")?.display;

		if (process.platform === "win32") {
			expect(actual).toContain(display);
			expect(actual).toMatch(/prime-agent\.ps1/i);
		} else {
	...
	test("installs a tarball spec without uninstalling the same logical package", () => {
		...
		expectSelfUpdateCommand(getSelfUpdateCommand("@earendil-works/pi-coding-agent", undefined, tarballUrl), {
			command: "npm",
	...
	test("installs a renamed tarball package before uninstalling the old one", () => {
		...
		expectSelfUpdateCommand(
			getSelfUpdateCommand("@earendil-works/pi-coding-agent", undefined, tarballUrl, "prime-agent"),
			{
				command: "npm",
				args: ["--prefix", prefix, "install", "-g", tarballUrl],
	...
	test("renames an npm global install by removing the old package first", () => {
		const { prefix } = createNpmPrefixInstall();

		expectSelfUpdateCommand(getSelfUpdateCommand("@mariozechner/pi-coding-agent", undefined, "@new-scope/pi"), {
			command: "npm",
	...
	// The pnpm, yarn, and bun fixtures answer their probes with a fake CLI on PATH, which Windows does not resolve.
	test.skipIf(process.platform === "win32").each<[string, () => void, string, string[], string[]]>([
		["pnpm", () => createPnpmGlobalInstall(), "pnpm", ["remove", "-g"], ["install", "-g"]],
		["yarn", () => createYarnGlobalInstall(), "yarn", ["global", "remove"], ["global", "add"]],
		["bun", () => createBunGlobalInstall(), "bun", ["uninstall", "-g"], ["install", "-g"]],
		...
		expect(result?.steps?.map((step) => ({ command: step.command, args: step.args }))).toEqual([
			{ command, args: [...remove, "@mariozechner/pi-coding-agent"] },
			{ command, args: [...install, "@new-scope/pi"] },
		]);
	});
});
