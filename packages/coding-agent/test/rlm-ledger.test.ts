...
describe("rlm spawn ledger", () => {
	it("replays spawn, rename, and delete records last-writer-wins", async () => {
		...
		try {
			...
			expect(statSync(ledger.ledgerPath).mode & 0o777).toBe(process.platform === "win32" ? 0o666 : 0o600);
			expect(statSync(dirname(ledger.ledgerPath)).mode & 0o777).toBe(process.platform === "win32" ? 0o666 : 0o700);
		} finally {
