...
describe("McpManager", () => {
	...
	// The name-based /mcp predicate has no manager instance, so it is pinned here: a leftover grant bound to an old
	// endpoint, and an unbound one, both read as disconnected.
	it("treats leftover and unbound OAuth credentials as disconnected for UI status", () => {
		authStorage.set("mcp:unbound", grant("unbound-token"));
		authStorage.set("mcp:remote", grant("old-token", "https://old.test/mcp"));
		authStorage.set("mcp:bound", grant("ok", "https://ok.test/mcp"));
		const userServers: Record<string, McpServerConfig> = {
			...
			bound: { type: "http", url: "https://ok.test/mcp", oauth: true },
		};
