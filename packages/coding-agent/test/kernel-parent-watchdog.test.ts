...
function writeFakePython(script: string[]): string {
	const python = join(tempDir, process.platform === "win32" ? "python.cmd" : "python");
	writeFileSync(python, script.join(process.platform === "win32" ? "\r\n" : "\n"));
...
describe("kernel parent watchdog", () => {
	...
	it.skipIf(process.platform === "win32")("direct spawn sets JPY_PARENT_PID and journals the kernel pid", async () => {
		const envDump = join(tempDir, "kernel-env");
		const python =
			process.platform === "win32"
				? writeFakePython(["@echo off", `set > "${envDump}"`, "exit /b 42"])
				: writeFakePython(["#!/bin/sh", `env > "${envDump}"`, "exit 42", ""]);
	...
	it("fork request env does not carry JPY_PARENT_PID (forked children watch the forkserver)", async () => {
		...
		try {
			await manager.execute("x").catch(() => undefined);
		} finally {
			errorSpy.mockRestore();
			await manager.dispose();
		}
