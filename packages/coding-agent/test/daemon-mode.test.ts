...
let testSocketSequence = 0;

function testSupervisorSocketPath(directory: string, fallbackName = "d.sock"): string {
	if (process.platform !== "win32") {
		return join(directory, fallbackName);
	}
	testSocketSequence++;
	return `\\\\.\\pipe\\prime-agent-daemon-mode-${process.pid}-${Date.now()}-${testSocketSequence}`;
}

function linkDirectory(realDir: string, aliasDir: string): void {
...
describe("daemon mode helpers", () => {
	...
	it("canonicalizes symlinked paths in the family catalog and name reservations", async () => {
		...
		try {
			...
			mkdirSync(realDir);
			linkDirectory(realDir, aliasDir);
	...
	it("does not retry supervisor agent-message rejections", async () => {
		...
		const socketPath = testSupervisorSocketPath(tempDir);
	...
	it("routes worker-local session renames through the supervisor", async () => {
		...
		const socketPath = testSupervisorSocketPath(tempDir, "s");
