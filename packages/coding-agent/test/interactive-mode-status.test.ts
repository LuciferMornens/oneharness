...
describe("InteractiveMode MCP command", () => {
	// An OAuth grant counts only while it is bound to the endpoint it was issued for:
	// leftover and unbound credentials must read as not connected in the /mcp output.
	type McpCommandHarness = {
		modelRegistry: { authStorage: { get(providerId: string): unknown; logout(providerId: string): void } };
