...
async function fixture() {
	...
	// applyThinkingLevel re-reads the effort the agent settled on, so the picker flow
	// needs a connection that reports back the level it just accepted.
	const getState = vi.fn(async () => ({
		thinkingLevel: setThinkingLevel.mock.lastCall?.[0] ?? "off",
		availableThinkingLevels: ["off", "low", "medium", "high"] as ModelThinkingLevel[],
	}));
	const mode = Object.assign(Object.create(InteractiveMode.prototype) as object, {
		...
		agentConnection: { setThinkingLevel, getState },
