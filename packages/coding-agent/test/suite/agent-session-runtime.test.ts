...
describe("AgentSessionRuntime characterization", () => {
	...
	it("keeps semantic spawn lineage through the production runtime factory", async () => {
		...
		// os.homedir() reads HOME on POSIX and USERPROFILE on Windows; stub both so
		// the prime CLI config path resolves inside the temp agent dir on either host.
...
describe("ENG-4620 fast mode settings", () => {
	...
	it.each([
		...
		// ChatGPT-subscription models already start fast, so asking for "priority"
		// alone changes nothing and saves nothing; toggle off first to record an
		// explicit preference on every model in this table.
		current.session.setServiceTier("default");
	...
	// The shared it.each body above toggles fast mode off before asking for it,
	// so it no longer exercises the fork default: ChatGPT-subscription models
	// start fast with nothing saved, and a clamp must not write a preference.
	it("clamps and restores the implicit fast default without saving a preference", async () => {
		const current = await createFastModeHarness(["gpt-5.4", "gpt-5.3"]);

		expect(current.session.serviceTier).toBe("priority");
		expect(current.settingsManager.getConfiguredDefaultServiceTier()).toBeUndefined();

		await current.session.setModel(current.getModel("gpt-5.3")!);
		expect(current.session.serviceTier).toBe("default");

		await current.session.setModel(current.getModel("gpt-5.4")!);
		expect(current.session.serviceTier).toBe("priority");
		expect(current.settingsManager.getConfiguredDefaultServiceTier()).toBeUndefined();
	});
	...
	it("uses the saved preference for new sessions and preserves it across tree navigation", async () => {
		...
		// gpt-5.4 on the ChatGPT subscription starts fast, so toggle off first to
		// make the following call an explicit, saved preference.
		current.session.setServiceTier("default");
		current.session.setServiceTier("priority");
