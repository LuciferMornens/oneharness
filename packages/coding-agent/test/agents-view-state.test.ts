...
// Windows refuses symlinkSync's "file" and "dir" types without the symlink
// privilege, so path-resolution coverage links directories as junctions there.
...
describe("agents view state", () => {
	describe("session identity caches", () => {
		...
		test("reuses successful canonical paths across reconciliation and row construction", () => {
			...
			const real = join(realDirectory, "session.jsonl");
			writeFileSync(real, "");
			symlinkSync(realDirectory, aliasDirectory, DIRECTORY_LINK_TYPE);
			const alias = join(aliasDirectory, "session.jsonl");
		...
		test.each(["missing", "symlink"] as const)("refreshes a cached %s after a fixed one-minute TTL", (kind) => {
			...
			const first = join(firstDirectory, "session.jsonl");
			const second = join(secondDirectory, "session.jsonl");
			const alias = join(aliasDirectory, "session.jsonl");
			writeFileSync(first, "");
			writeFileSync(second, "");
			if (kind === "symlink") symlinkSync(firstDirectory, aliasDirectory, DIRECTORY_LINK_TYPE);
			...
			if (kind === "symlink") rmSync(aliasDirectory, { recursive: true, force: true });
			symlinkSync(secondDirectory, aliasDirectory, DIRECTORY_LINK_TYPE);
		...
		test("refreshes cached roster IDs after their parent symlink changes and the TTL expires", () => {
			...
			mkdirSync(firstDirectory);
			mkdirSync(secondDirectory);
			writeFileSync(join(firstDirectory, "parent.jsonl"), "");
			writeFileSync(join(secondDirectory, "parent.jsonl"), "");
			const alias = join(aliasDirectory, "parent.jsonl");
			symlinkSync(firstDirectory, aliasDirectory, DIRECTORY_LINK_TYPE);
	...
	test("deduplicates and protects sessions across symlink aliases", () => {
		...
		try {
			...
			writeFileSync(real, "");
			symlinkSync(realDir, aliasDir, DIRECTORY_LINK_TYPE);
	...
	test("hides abandoned message-less drafts but keeps saved sessions as enrichment for live rows", () => {
		...
		const namedDraft = makeSessionInfo({
		...
		// Drafts stay in the catalog so ancestry, anchors and enrichment survive;
		// only the view filter drops the abandoned, unnamed ones.
		const records = reconcileUnifiedSessions([], [draft, namedDraft, conversation]);
		expect(records.map((record) => record.saved?.id)).toEqual(["draft", "named", "talk"]);
		expect(filterEmptyAgentsViewSessions(records).map((record) => record.saved?.id)).toEqual(["named", "talk"]);
