...
describe("buildSnapshotCode", () => {
	...
	it("excludes oversized variables from persistent snapshots and deletes them when pruning", () => {
		expect(code).toContain("identify_oversized = False");
		expect(code).toContain("exceeds per-variable snapshot size cap");
		expect(code).not.toContain("identify_oversized = True");

		const pruneCode = buildSnapshotCode(
			"/state/sess.dill",
			"/state/sess.json",
			DEFAULT_SNAPSHOT_MAX_BYTES,
			DEFAULT_SNAPSHOT_MAX_VARIABLE_BYTES,
		...
		expect(pruneCode).toContain("identify_oversized = True");
		expect(pruneCode).toContain("del ns[name]");
		expect(pruneCode).toContain("pruned_ids");
	});
});
