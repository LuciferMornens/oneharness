...
import { getApiProvider, getModels, streamSimple } from "@earendil-works/pi-ai";
...
describe("ModelRegistry", () => {
	...
	describe("custom models merge behavior", () => {
		...
		const reasoningModel = {
			thinkingLevelMap: { minimal: null, high: "max" },
			reasoningCapabilities: { control: "budget", levels: { low: 1024, high: 8192, max: 32768 } },
		};
		...
		test.each<[string, Record<string, unknown>, Record<string, unknown>, Record<string, unknown>]>([
			...
			[
				"the schema preserves reasoning capabilities alongside a legacy thinking map",
				{},
				reasoningModel,
				reasoningModel,
			],
		...
		function orcaRouterModelsJson(modelCompat: Record<string, unknown>, providerCompat?: Record<string, unknown>) {
			writeRawModelsJson({
				orcarouter: {
					...
					models: [{ id: "orcarouter/auto", name: "OrcaRouter Auto", compat: modelCompat }],
				},
			});
		}

		test.each<[string, Record<string, unknown>, Record<string, unknown> | undefined, Record<string, unknown>]>([
			[
				"keeps a model-level orcaRouterRouting block",
				{ orcaRouterRouting: { models: ["openai/gpt-4o", "anthropic/claude-sonnet-4"], route: "fallback" } },
				undefined,
				{ models: ["openai/gpt-4o", "anthropic/claude-sonnet-4"], route: "fallback" },
			],
			[
				"prefers model-level orcaRouterRouting fields over provider-level ones",
				{ orcaRouterRouting: { models: ["google/gemini-2.5-pro"] } },
				{ orcaRouterRouting: { models: ["openai/gpt-4o", "anthropic/claude-sonnet-4"], route: "fallback" } },
				{ models: ["google/gemini-2.5-pro"], route: "fallback" },
			],
		])("%s", (_name, modelCompat, providerCompat, expected) => {
			orcaRouterModelsJson(modelCompat, providerCompat);

			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
			const compat = registry.find("orcarouter", "orcarouter/auto")?.compat as
				| OpenAICompletionsCompatWithOrca
				| undefined;

			expect(registry.getError()).toBeUndefined();
			expect(compat?.orcaRouterRouting).toEqual(expected);
		});
