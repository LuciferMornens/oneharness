...
describe("daemon supervisor ownership registry", () => {
	...
	it("disambiguates never-acquired from lost-on-disk ownership errors", async () => {
		...
		expect(lostOnDisk.message.toLowerCase()).toContain(paths.socketPath.toLowerCase());
		expect(lostOnDisk.message.toLowerCase()).toContain(paths.registryDir.toLowerCase());
