...
- Claude Pro/Max
- GitHub Copilot
- xAI Grok (SuperGrok / Premium+) — Grok CLI proxy, provider `grok`
- xAI (Grok) — eligible xAI subscriptions, provider `xai`
...
`/login` offers two xAI subscription entries. They take different routes, use different provider IDs, and store separate `auth.json` keys; connect either or both.

#### xAI Grok (SuperGrok / Premium+) — provider `grok`

- Requires an active SuperGrok subscription (grok.com) or an X Premium+ subscription on the linked X account
...
Use `/login` and select the `xai` subscription entry (listed as "xAI (Grok)"), then open the printed verification URL and enter the shown code to authorize an eligible Grok subscription. The existing xAI API-key entry still accepts a key, and `XAI_API_KEY` remains supported.

Both `xai` routes, subscription and API key, use provider ID `xai` and `https://api.x.ai/v1`. Subscription requests use `/responses`; API-key requests keep the existing `/chat/completions` route and model defaults. Changing authentication updates the current session without requiring model reselection.
