...
- `thinking`: an explicit child reasoning level (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`) that the resolved child model must support; omitted, it inherits the parent level clamped to the child model.

The host accepts `effort` as an equivalent spelling of that level on the `rlm.run` host request and rejects a call that sends both with different values. The Python keyword is `thinking`.
