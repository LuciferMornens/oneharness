...
| `ORCAROUTER_API_KEY` | OrcaRouter API key; `ORCA_KEY` is accepted as a fallback |
...
| `PRIME_AGENT_KERNEL_PYTHON` | Use an existing Python environment with `prime-agent-runtime` instead of bootstrapping a venv under `~/.prime/agent/kernel-venvs/` |
