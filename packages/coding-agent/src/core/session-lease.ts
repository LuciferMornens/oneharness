...
import { execFileSyncHidden, isProcessAlive, WINDOWS_POWERSHELL_EXECUTABLES } from "../utils/child-process.js";
...
export function getWindowsProcessStartId(pid: number, query: ProcessQuery = runProcessQuery): string | undefined {
	...
	const executables = query === runProcessQuery ? WINDOWS_POWERSHELL_EXECUTABLES : (["powershell.exe"] as const);
	for (const executable of executables) {
		try {
			const startTicks = query(executable, [
				"-NoLogo",
				"-NoProfile",
				"-NonInteractive",
				"-Command",
			...
			// Try the Windows PowerShell host when PowerShell 7 is unavailable.
		}
	}
	return undefined;
}
