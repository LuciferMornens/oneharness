...
export class AgentSession {
	...
	private async _startRlmChildRun(
		...
		const { name: rawName, model: rawModel, effort: rawEffort, thinking: rawThinking, ...unsupported } = kwargs;
		...
		if (
			...
			requestedEffort !== requestedThinkingLevel
		) {
			throw new Error(`rlm.run effort and thinking disagree: "${requestedEffort}" vs "${requestedThinkingLevel}"`);
		}
		const requestedLevel = requestedEffort ?? requestedThinkingLevel;
		const thinkingLevel = resolveRlmSubagentThinkingLevel(
			modelSelection.model,
			this.thinkingLevel,
			requestedLevel,
			requestedEffort !== undefined ? "effort" : requestedThinkingLevel !== undefined ? "thinking" : "effort",
		);
