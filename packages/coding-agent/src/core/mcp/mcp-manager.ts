...
import { getCatalogEntry, registerBuiltinMcpOAuthProviders } from "@earendil-works/pi-ai/mcp";
...
/**
 * Name-based connection predicate for the legacy /mcp command surface, which
 * resolves a bare server name without a manager instance. It applies the same
 * rules dispatch eligibility does: a reserved catalog name is served only from
 * its canonical declaration, a configured env var is the only bearer source
 * when set, and an OAuth grant counts only while it is bound to the endpoint it
 * was issued for — leftover or unbound credentials read as disconnected.
 */
export function isMcpServerConnected(
	...
	const reserved = getCatalogEntry(server);
	const config = userServers?.[server];
	if (config?.enabled === false) return false;
	if (config && !reserved) {
		if (config.type === "stdio") return true;
		// The configured env var is the ONLY credential source for this server: when
		// it is unset, a stale OAuth credential stored under the same id must never
		// read as connected.
		if (config.bearerTokenEnvVar) return Boolean(process.env[config.bearerTokenEnvVar]?.trim());
		if (config.oauth !== true) return true;
		return oauthGrantUsable(authStorage.get(mcpCredentialKey(server)), config.url).usable;
	}
	if (config && reserved) {
		// A bundled catalog service owns its name, so a shadowing user entry is dead
		// by design and its token can never replay against the official endpoint.
		// The one exception is the canonical declaration, which `reservedMcpOwnership`
		// keeps live for dispatch: it names the official endpoint with no override, so
		// it falls through to the catalog rule below — the integration the manager
		// actually serves. Mirror of that ownership rule; keep the two in step.
		if (
			config.type !== "http" ||
			config.url !== reserved.url ||
			config.oauth !== true ||
	...
	const credential = authStorage.get(mcpCredentialKey(server));
	// Ids outside the reserved catalog (per-account connections, imported services)
	// have no endpoint to bind against here; the manager owns their full status, so
	// credential presence is all this predicate can honestly report. OAuth support is
	// keyed off the auth strategy, the same signal the manager resolves usesOAuth from.
	if (!reserved || (reserved.auth.strategy !== "oauth" && reserved.auth.strategy !== "unknown")) {
		return credential !== undefined;
	}
	return oauthGrantUsable(credential, reserved.url).usable;
}
