...
export class ModelRegistry {
	...
	private getXaiSubscriptionModel(model: Model<Api>): Model<Api> | undefined {
		...
		// The adapter replaces compat wholesale, which would drop catalog and models.json flags.
		const adapted: Model<Api> = { ...subscription, compat: mergeCompat(source.compat, subscription.compat) };
		this.xaiModelSources.set(adapted, source);
		return adapted;
	}
