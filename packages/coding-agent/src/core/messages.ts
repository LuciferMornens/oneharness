...
/**
 * Body blocks for a child that ended its run without replying. A plain "stop" is the ordinary
 * end of a turn and says nothing about the silence, so only the other stop reasons are reported;
 * their prose never enters the `[child-exited: ...]` header, which stays machine-parsable.
 */
function formatNoReplyNoticeBody(
	details: Extract<RlmChildTerminalNoticeDetails, { kind: "completed_without_reply" }>,
): string {
	const blocks: string[] = [];
	if (details.lastStopReason && details.lastStopReason !== "stop") {
		blocks.push(`Last stop reason: ${describeStopReason(details.lastStopReason)}`);
	}
	if (details.lastAssistantTextPreview) {
		blocks.push(`Last assistant text: ${details.lastAssistantTextPreview}`);
	}
...
export function createRlmChildTerminalNoticeMessage(
	...
	const content =
		details.kind === "cancelled"
			...
			: `[child-exited: no-reply child:${childName}]${formatNoReplyNoticeBody(details)}`;
	return {
		role: "custom",
