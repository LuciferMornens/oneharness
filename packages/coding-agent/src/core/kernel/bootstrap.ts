...
// recorded on disk, keyed by import name like the manifest itself. A missing or
// corrupt marker contributes no base entries.
async function writeMergedBootstrapVersion(
	...
	const merged = pythonSkillsByImportName(version?.pythonSkills);
	for (const skill of pythonSkills) {
		merged.set(skill.importName, skill);
	}
	await writeBootstrapVersion(venv, runtimeIdentity, sortPythonSkillsForManifest(merged.values()));
}
...
async function syncPythonSkills(
	...
	for (const skill of sortPythonSkillsForInstall(pythonSkills)) {
		...
		// Checkpoint after every completed install so a killed session resumes at
		// the first missing skill instead of re-syncing from scratch.
		await writeMergedBootstrapVersion(venv, runtimeIdentity, installedPythonSkills);
	}
	// Record the full set once the sync is done. The write merges rather than
	// replaces: skills installed by other sessions stay recorded (nothing is ever
	// uninstalled), while a reinstall replaces that import name's entry.
	await writeMergedBootstrapVersion(venv, runtimeIdentity, installedPythonSkills);
}
...
async function ensureKernelPythonUncached(
	...
	// No-skill callers (postinstall, runtime-bootstrap, bootstrap-cli) never sync
	// skills and only need the base kernel, so they check for that directly and
	// never reach the skill sync with an empty skill list.
	...
	if (await readyForCaller()) return finish();
