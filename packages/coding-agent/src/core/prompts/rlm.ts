...
export function buildRlmPrompt(options: RlmPromptOptions): string {
	...
	if (allowRecursion && hasIpython) {
		parts.push(
			...
			"Choose a spawn-time reasoning level with `thinking='xhigh'` (or `off`/`minimal`/`low`/`medium`/`high`/`max`) when the selected model lists that level. An unavailable model or unsupported level fails spawn; decide whether to retry or omit `model`/`thinking`.",
		);
		parts.push(
			hasAgentObserve
