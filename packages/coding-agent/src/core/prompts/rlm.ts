...
export function buildRlmPrompt(options: RlmPromptOptions): string {
	...
	if (allowRecursion && hasIpython) {
		parts.push(
			...
			"Choose spawn-time effort with `effort='xhigh'` (or `off`/`minimal`/`low`/`medium`/`high`/`max`) when the selected model lists that level. `thinking` is an alias for `effort`. An unavailable model or unsupported effort fails spawn; decide whether to retry or omit `model`/`effort`/`thinking`. Unknown `rlm.run` options fail.",
		);
		if (hasAgentMessage) {
			parts.push(
