...
export class InteractiveMode {
	...
	private applyModelSwitchUiState(
		state: Pick<AgentConnectionState, "model" | "thinkingLevel" | "serviceTier" | "availableThinkingLevels">,
		fallbackModel: AgentConnectionModel,
	): void {
		this.patchConnectionState({
			model: state.model ?? fallbackModel,
			// The agent resolves the thinking level for the new model; mirror the
			// resolved value instead of leaving the previous model's level on screen.
			thinkingLevel: state.thinkingLevel,
	...
	private applyThinkingLevel(level: ThinkingLevel, showConfirmation = true): Promise<boolean> {
		const connection = this.agentConnection;
		const sessionId = this.connectionState?.sessionId;
		return connection
			...
			.then((state) => {
				...
				// The agent may resolve the request to a different level; report what it settled on.
				this.patchConnectionState({
					thinkingLevel: state.thinkingLevel,
					availableThinkingLevels: state.availableThinkingLevels,
				});
			...
			const authenticate = (provider: AuthSelectorProvider, tab: "providers" | "mcp-connections") => {
				...
				void authFlows
					...
					.then(async (authResult) => {
						...
						// A deferred catalog refresh has no agent connection to ask yet;
						// the next connection picks the new provider up.
