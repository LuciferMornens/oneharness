		...
		switch (command.type) {
			...
			case "delete_saved_session":
				if (!command.activeSessionId) {
					...
					if (
						this.sessionFileIsLiveResident(command.sessionPath) ||
						entry?.summary.activeSessionId !== undefined
					) {
		...
		if (orphanProcessJournalPath) {
			try {
				...
				for (const orphan of orphans) {
					...
					if (process.platform === "win32") {
						if (!orphan.processStartId) {
							reapFailed = true;
							continue;
						}
						const result = await terminateWindowsProcessTreeByIdentity(orphan.pid, orphan.processStartId);
						if (result === "failed") reapFailed = true;
					} else if (orphan.processStartId) {
